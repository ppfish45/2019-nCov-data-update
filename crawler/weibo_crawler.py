...
from tools.common import is_scanned, add_scanned
...
class Weibo(object):
    ...
    def print_user_info(self):
        """打印用户信息"""
        print(f"正在爬取 {self.user['screen_name']} 的微博 ...")
    ...
    def get_filepath(self, type):
        ...
        try:
            ...
            if type == 'img' or type == 'video':
                return file_dir
            file_path = file_dir + os.sep + self.user['screen_name'] + '.' + type
            return file_path
        except Exception as e:
            print('Error: ', e)
            traceback.print_exc()
    ...
    def start(self):
        ...
        try:
            for user_config in self.user_config_list:
                ...
                self.get_user_info()
                if is_scanned(self.user['screen_name']):
                    print(f"跳过 {self.user['screen_name']} 的微博，因为已经爬取过。")
                    continue
                self.get_pages()
                print(u'信息抓取完毕')
                print('*' * 100)
                ...
                add_scanned(self.user['screen_name'])
        except Exception as e:
            print('Error: ', e)
            traceback.print_exc()


def main(save_path=os.path.split(os.path.realpath(__file__))[0]):
    try:
        config_path = os.path.split(
            os.path.realpath(__file__))[0] + os.sep + 'weibo_config.json'
        if not os.path.isfile(config_path):
            sys.exit(u'当前路径：%s 不存在配置文件weibo_config.json' %
                     (os.path.split(os.path.realpath(__file__))[0] + os.sep))
        with open(config_path) as f:
            try:
                config = json.loads(f.read())
            except ValueError:
                sys.exit(u'weibo_config.json 格式不正确，请参考 '
                         u'https://github.com/dataabc/weibo-crawler#3程序设置')
        wb = Weibo(config,
                   save_path)
        wb.start()  # 爬取微博信息
